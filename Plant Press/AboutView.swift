        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 30) {
                    ...
                    // --- 2. LEFT-ALIGNED BODY ---
                    // This single VStack handles the layout for all the text inside it!
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        Text("When you create a new observation, you will be asked to provide the genus. If the species is unknown, you can select sp., cf., or ? for unknown. Dropdown lists allow you to select pre-specified genera and species names from an internal list of New York State plants. An optional variety and subspecies dialogue is also provided. The location can either be specified with the map or left blank. If left blank, it will be assigned to the site location. You can also add any number of photos and notes before saving the observation.")
                    ...
                    // Forces the invisible box to fill the screen width, anchoring the text to the left margin
